...
class Group:
    def __init__(self, name: str, url: str, session: HTTPSession):
        self.name = name
        self.url = url
...
class Sampler:
    ...
    @classmethod
    def scrape_groups(cls, session: HTTPSession) -> "Sampler":
        ...
        return Sampler(session, [Group(group.string, group.a["href"], session) for group in groups])

    def get_sample(self) -> Company:
...
def run_sampling(num_samples: int, append: bool, output_path: Path):
    ...
    with output_path.open("a" if append else "w", newline="") as email_out:
        ...
        with httpx.Client() as client:
            sampler = Sampler.scrape_groups(HTTPSession(client, 0.1))
...
def main():
    ...
    run_sampling(args.num_samples, args.append, args.output)
